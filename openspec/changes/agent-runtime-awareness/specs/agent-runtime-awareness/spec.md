...
#### Scenario: Tool reflects current policy classification

- **WHEN** an agent calls the runtime introspection tool
- **THEN** the response includes any relevant runtime policy classification used to determine its mutation permissions

### Requirement: Non-build agents are denied mutation-capable tools at runtime

The system SHALL enforce non-build agent boundaries at tool-execution time so planning and review workflows cannot modify files or execute arbitrary implementation shell commands through prompt bypasses.

#### Scenario: Planning agent cannot mutate files

- **WHEN** a non-build planning agent attempts to use a file-modifying tool such as edit, write, or patch
- **THEN** the tool execution is denied by runtime policy

#### Scenario: Planning agent cannot execute arbitrary implementation shell commands

- **WHEN** a non-build planning agent attempts to execute a shell command outside the allowed analysis workflow
- **THEN** the shell execution is denied by runtime policy

#### Scenario: Build agent retains implementation access

- **WHEN** the active agent is classified as a build agent
- **THEN** the runtime awareness plugin does not block its normal file-mutation and shell-based implementation workflows
